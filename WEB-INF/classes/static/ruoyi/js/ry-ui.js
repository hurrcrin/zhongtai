    ...
    $.extend({
        ...
        table: {
            ...
            queryParams: function (params) {
                console.log("queryParams params:" + JSON.stringify(params));
                var curParams = {
                    // 传递参数查询参数
            ...
            exportDetailExcel: function (formId) {
                table.set();
                $.modal.confirm("确定导出明细数据吗？", function () {
                    var currentId = $.common.isEmpty(formId) ? $('form').attr('id') : formId;
                    $.modal.loading("正在导出数据，请稍后...");
                    $.post(table.options.exportDetailUrl, $("#" + currentId).serializeArray(), function (result) {
                        if (result.code == web_status.SUCCESS) {
                            window.location.href = ctx + "common/download?fileName=" + encodeURI(result.msg) + "&delete=" + true;
                        } else if (result.code == web_status.WARNING) {
                            $.modal.alertWarning(result.msg)
            ...
            // 导出选择项数据
            exportSelectedExcel: function (formId) {
                table.set();
                $.modal.confirm("确定导出所有选中项asin的评论吗？", function () {
                    var arrays = $.table.selectColumns("asin");
                    $.modal.loading("正在导出数据，请稍后...");
                    $.post(table.options.exportSelectedUrl, {asins: JSON.stringify(arrays)}, function (result) {
                        if (result.code == web_status.SUCCESS) {
                            window.location.href = ctx + "common/download?fileName=" + encodeURI(result.msg) + "&delete=" + true;
                        } else if (result.code == web_status.WARNING) {
                            $.modal.alertWarning(result.msg)
            ...
            importExcel_SonKi: function (formId) {
                table.set();
                var currentId = $.common.isEmpty(formId) ? 'importTpl' : formId;
                layer.open({
                    type: 1,
                    ...
                    btn1: function (index, layero) {
                        ...
                        var sellerId = $("#sellerId1").val();
                        if (sellerId == null || sellerId == '') {
                            $.modal.msgWarning("导入前请选择账号！");
                            return false;
                        }
                        var index = layer.load(2, {shade: false});
                        $.modal.disable();
        ...
        modal: {
            ...
            // 消息提示并刷新父窗体
            msgReloadWithoutReload: function (msg, type) {
                layer.msg(msg, {
                    icon: $.modal.icon(type),
                    time: 500,
                    shade: [0.1, '#8F8F8F']
        ...
        operate: {
            ...
            skip: function (id, status) {
                var url = "/ZhongTai/supplychain/invoiceplandetail?mainid=" + id + "&status=" + status;
                $.modal.openFull("开票计划明细数据", url);
            },
            // 修改信息
            edit: function (id) {
                table.set();
